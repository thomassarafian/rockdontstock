class SneakersController < ApplicationController
	...
	def index
		@selection_of_the_day = Sneaker.highlighted.last || Sneaker.selected.last || Sneaker.where('state >= ?', 0).first
		results = Sneaker.where('state >= ?', 0).search_by_name_and_brand(params[:search])
